...
export class AddMoviePage implements OnInit {
  ...
  ionViewWillEnter() {
    this.movieService.getGenres(this.user).subscribe((genres) => {
      // this.genres = genres;
    });
  ...
  onAddMovie(form: NgForm) {
    if (form.valid) {
      this.movieToAdd = new Movie(null, form.value.name, form.value.producer, form.value.image, form.value.description, form.value.year,
        form.value.country, form.value.genre, form.value.budget, form.value.duration, 0,0);

      console.log(this.movieToAdd.genre.name);
      this.movieService
        .addMovie(this.movieToAdd, this.user)
        .subscribe(movies => {
          movies.map((movie) => {
